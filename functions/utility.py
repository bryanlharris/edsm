...
def get_function(path):
    ...
    ``path`` may be a fully qualified name such as
    ``"functions.read.stream_read_cloudfiles"`` or a bare function
    name that exists in the current module.  If no ``'.'`` is present in
    ``path`` the lookup falls back to ``globals()`` before attempting to
    import a module.
    """
